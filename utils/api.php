...
class API {
     ...
     * for the "flashi-idToken" and "flashi-refreshToken" with the corresponding values
    ...
    private function updateCookies($response) {
        if (isset($response["idToken"]) && isset($response["refreshToken"])) {
            /**
             * Sets a cookie with the provided authentication token.
             *
             * @param string $name The name of the cookie.
             * @param string $value The value of the cookie.
             * @param int $expire The expiration time of the cookie in Unix timestamp format.
             * @param string $path The path on the server for which the cookie will be available.
             * @param string $domain The domain for which the cookie will be available.
             * @param bool $secure Indicates whether the cookie should only be transmitted over HTTPS.
             * @param bool $httponly Indicates whether the cookie should be accessible only through HTTP headers and not via JavaScript.
             */
            setcookie("flashi-idToken", $response["idToken"], time() + 3600 * 24, "/", "", false, true); // Cookie will expire in 1 day
            setcookie("flashi-refreshToken", $response["refreshToken"], time() + 3600 * 24, "/", "", false, true); // Cookie will expire in 1 day
        }
    }
    ...
    public function getToken() {
        switch ($_SERVER['REQUEST_METHOD']) {
            ...
            default:
                if (isset($_COOKIE['flashi-idToken'])) {
                    return htmlspecialchars($_COOKIE['flashi-idToken']);
                }
        }
